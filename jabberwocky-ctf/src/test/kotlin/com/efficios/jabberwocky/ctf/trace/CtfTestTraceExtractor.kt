...
private constructor(tracePath: Path, private val parentPath: Path) : AutoCloseable {
    ...
    companion object {
        ...
        fun extractTestTrace(trace: CtfTestTrace): CtfTestTraceExtractor {
            val tracePath: Path
            try {
                val parentPath = Files.createTempDirectory("test-trace") //$NON-NLS-1$
        ...
        private fun extractUrlToTempDir(sourceURL: URL, destinationPath: Path): Path {
            val urlString = sourceURL.getFile()
            ...
            JarFile(jarFileName).use({ jar ->
                val allEntries = Collections.list(jar.entries())

                /* Get the "top-level" directory of the trace we want to extract */
                val entriesToExtract = allEntries.stream()
                        .filter({ entry -> entry.getName().startsWith(resourcePath + "/") }).collect(Collectors.toList())

                entriesToExtract.stream()
                        .sorted(Comparator.comparing(java.util.function.Function<JarEntry, String> { it.getName() }))
                .forEach { entry ->
